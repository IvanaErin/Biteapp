...
# ---------------------------
    ...
    if role == "Staff":
        ...
        # Sales Report
            ...
            if local:
                receipts = pd.concat([receipts, pd.DataFrame(local)], ignore_index=True) if not receipts.empty else pd.DataFrame(local)
    ...
    # ---------------------------
        ...
        with left_col:
            ...
            with st.expander("💬 Ask BiteHub AI", expanded=False):
                user_question = st.text_input("Ask about our meals, menu, or promos:", key="user_ai_q")
                ...
                if detected_item_col and detected_price_col:
                    ...
                    for cat in categories:
                        ...
                        for _, row in cat_items.iterrows():
                            ...
                            try:
                                price_val = float(row.get(detected_price_col, 0.0))
                            except Exception:
                                price_val = 0.0
        ...
        with right_col:
            ...
            if not is_guest:
                available_items = menu_df[detected_item_col].fillna("Unknown").tolist() if detected_item_col else menu_df.iloc[:, 0].fillna("Unknown").tolist()
...
# ---------------------------
    ...
    if not pending_cart:
        ...
        if method == "Cash" and st.button("Confirm Cash Payment"):
            ...
            save_receipt(order_id, pending_cart, total_cost, "Cash", user.get("username", "Guest"), pickup_dt, status="Pending")
            ...
            if st.button("✅ I've Paid via GCash"):
                order_id = f"ORD-{random.randint(100000,999999)}"
                save_receipt(order_id, pending_cart, total_cost, "GCash", user.get("username", "Guest"), pickup_dt, status="Pending")
                st.success("✅ Order recorded (Pending). Staff will mark it Ready when prepared.")
                st.session_state.cart.clear()
                st.session_state.page = "main"
                st.rerun()
            ...
            if st.button("Simulate Card Payment Success"):
                order_id = f"ORD-{random.randint(100000,999999)}"
                save_receipt(order_id, pending_cart, total_cost, "Card", user.get("username", "Guest"), pickup_dt, status="Pending")
                st.success("✅ Order recorded (Pending). Staff will mark it Ready when prepared.")
                st.session_state.cart.clear()
                st.session_state.page = "main"
                st.rerun()
