...
if st.session_state.page == "main":
    ...
    if role == "Staff":
        ...
        st.session_state.choice = st.sidebar.selectbox(
            "Menu",
            ["Home", "Pending Orders", "Manage Menu", "AI Assistant", "Feedback Review", "Sales Report"],
            index=0
        )
        ...
        try:
            menu_df = load_menu()
            menu_data = {} if menu_df.empty else {cat: dict(zip(g["ITEM"], g["PRICE"])) for cat, g in menu_df.groupby("CATEGORY")}
        except Exception as e:
    ...
    # ---------------------------
        ...
        # LEFT: AI Assistant + Menu
        with col1:
            st.subheader("🤖 AI Assistant")
            st.info("Guest users cannot ask AI questions. Sign up to enable this feature.")

            st.subheader("📖 Menu Ordering")
            menu_df = load_menu()
            st.dataframe(menu_df)
            st.info("Guest users can browse the menu but cannot place orders.")
        ...
        with col2:
            ...
            st.info("Guest users cannot post feedback. Sign up to participate.")

        # BELOW: Order history / notifications
        st.subheader("📦 Order History")
        receipts_df = load_receipts_df()
        if not receipts_df.empty:
            st.dataframe(receipts_df)
            st.info("Orders are not saved in Guest sessions.")
        else:
