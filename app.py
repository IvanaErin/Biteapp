...
def set_background(image_file: str | None = None):
    ...
    css_parts.append(
        """
        [data-testid="stAppViewContainer"] > section:first-child { padding-top: 18px !important; margin-top: 0px !important; }
        #MainMenu { visibility: hidden; }
        footer { visibility: hidden; }
        .login-card { background: rgba(10,10,10,0.6); padding: 1.6rem; border-radius: 12px; max-width: 840px; margin: 18px auto; color: #fff; box-shadow: 0 8px 28px rgba(0,0,0,0.5); }
        div.stButton > button { width: 100%; height: 44px; font-size: 15px; border-radius: 8px; }
        .stTextInput>div>div>input, .stTextInput>div>div>div>input { background: rgba(0,0,0,0.55); color: #fff; }
        .stContainer, .stMarkdown, .stExpander { color: #fff; }
        """
    )
...
def save_account(username: str, password: str, role: str = "Non-Staff"):
    ...
    if not conn:
        _ensure_local_db()
        st.session_state._local_accounts[username] = {"password": password, "role": role, "loyalty_points": 0}
        return
    try:
        cur = conn.cursor()
...
def get_account(username: str):
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT id, username, password, role, loyalty_points FROM users WHERE username=%s", (username,))
        row = cur.fetchone()
        if row:
...
if st.session_state.page == "login":
    st.markdown("<h1 style='text-align:center;color:#FF6F61; font-size:60px;'>☕ BiteHub</h1>", unsafe_allow_html=True)
...
# ---------------------------
    ...
    # ---------- NON-STAFF & GUEST PORTAL ----------
        ...
        # Two columns: Left for AI + Menu + Cart, Right for Sentiment + Feedback + Notifications
                ...
                for cat in menu_df["CATEGORY"].unique():
                    with st.expander(cat, expanded=True):
                        ...
                        for idx, row in cat_items.iterrows():
                            ...
                            qty = action_col.text_input("", "0", max_chars=2, key=qty_key)
        ...
        # RIGHT COLUMN (Sentiment, Feedback, Notifications)
        with col2:
